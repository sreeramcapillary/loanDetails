...
router.get('/getAllALoanDetailsList', function (request, response) {
	...
	connection.query('SELECT ld.id, ld.loan_id, ld.bucket, u.name FROM `loan_details` ld JOIN userdetails u Where u.id = ld.assigned_emp_id', function (error, results, fields) {
		if (results.length > 0) {
			//	request.session.loggedin = true;
			// request.session.username = username;
...
router.post('/getLoanListByEmpByDate', function (request, response) {
	...
	where u.id = ${empId} AND ld.batch_status = 1 AND ld.is_assigned = 1 GROUP BY ld.id ORDER BY lsh.statusId ASC, noOfComments ASC`, function (error, results, fields) {
		if (results.length > 0) {
			let responseData = { "status": true, "code": 200, "assignedLoanToEmp": results }
			response.json(responseData)
		} else {
	...
	if (loanDetails == null) {
		...
		if (filename) {
			...
			//	console.log(results.insertId)
				...
				let inserData = '';
				let queryTest
				let startQuery = "INSERT INTO `loan_details` (`client_id`,`imported_file_id`,`Customer_id`, `Loan_Count`, `loanid`, `customer_Name`, `Gender`, `mobile`,`email`, `DOB`, `Age`, `city`, `pin_code`, `state`, `loan_id`, `disbursal_amt`, `disbursal_date`,`due_date`, `principal_amt`, `interest_amount`, `penalty_amt`, `repayment_amt`, `ref_type1`, `ref_name1`, `ref_mobile_num1`, `ref_type2`, `ref_name2`, `ref_mobile_num2`, `current_address`, `permanent_address`, `bucket`,`overdue_days`, `is_collected`, `ESIGN_MOBILE_NUMBER`, `repaid_date`,`is_assigned`,`date`) VALUES";
				...
				loanDetails.map(item => {
					...
					'${item.ref_name1}','${item.ref_mobile_num1}' , '${item.ref_type2}','${item.ref_name2}' ,'${item.ref_mobile_num2}', '${item.current_address}', '${item.permanent_address}' ,
...
router.post('/getDayReport', async(request, response) => {
	...
	if(batch == 1){
		...
		if(role[0].usertype == 1 || role[0].usertype == 3){
			...
			COUNT(CASE WHEN LD.is_assigned = 1 THEN LD.principal_amt ELSE NULL END) as assignedAmount_COUNT, 
			
			COALESCE(SUM(CASE WHEN lsh.statusId = 1 THEN LD.principal_amt END), 0) as PTP_AMOUNT, 
			
			COUNT(CASE WHEN lsh.statusId = 1 THEN lsh.statusId ELSE NULL END) as PTP_AMOUNT_COUNT, 
			
			COALESCE(SUM(CASE WHEN lsh.statusId = 2 THEN LD.principal_amt END), 0) as RNR_AMOUNT, 
			
			COUNT(CASE WHEN lsh.statusId = 2 THEN lsh.statusId ELSE NULL END) as RNR_AMOUNT_COUNT, 
			
			COALESCE(SUM(CASE WHEN lsh.statusId = 3 THEN LD.principal_amt END), 0) as SWITCH_OFF, 
			
			COUNT(CASE WHEN lsh.statusId = 3 THEN lsh.statusId ELSE NULL END) as SWITCH_OFF_COUNT, 
			
			COALESCE(SUM(CASE WHEN lsh.statusId = 4 THEN LD.principal_amt END), 0) as PAYMENT_EXPECTED_AT, 
			
			COUNT(CASE WHEN lsh.statusId = 4 THEN lsh.statusId ELSE NULL END) as PAYMENT_EXPECTED_AT_COUNT, 
			
			COALESCE(SUM(CASE WHEN lsh.statusId = 5 THEN LD.principal_amt END), 0) as WAITING_FOR_CONFIRMATION, 
			
			COUNT(CASE WHEN lsh.statusId = 5 THEN lsh.statusId ELSE NULL END) as WAITING_FOR_CONFIRMATION_COUNT, 
			
			COALESCE(SUM(CASE WHEN lsh.statusId = 6 THEN LD.principal_amt END), 0) as collectedAmout, 
			
			COUNT(CASE WHEN lsh.statusId = 6 THEN lsh.statusId ELSE NULL END) as collectedAmout_COUNT, 
			
			COALESCE(SUM(CASE WHEN lsh.statusId = 7 THEN LD.principal_amt END), 0) as NRAmout, 
			
			COUNT(CASE WHEN lsh.statusId = 7 THEN lsh.statusId ELSE NULL END) as NRAmout_COUNT, 
			
			COALESCE(SUM(CASE WHEN lsh.statusId = 8 THEN LD.principal_amt END), 0) as NCAmout, 
			
			COUNT(CASE WHEN lsh.statusId = 8 THEN lsh.statusId ELSE NULL END) as NCAmout_COUNT, 
			
			COALESCE(SUM(CASE WHEN lsh.statusId = 9 THEN LD.principal_amt END), 0) as INAmout, 
			
			COUNT(CASE WHEN lsh.statusId = 9 THEN lsh.statusId ELSE NULL END) as INAmout_COUNT, 
			
			COALESCE(SUM(CASE WHEN lsh.statusId = 10 THEN LD.principal_amt END), 0) as RTPAmout, 
			
			COUNT(CASE WHEN lsh.statusId = 10 THEN lsh.statusId ELSE NULL END) as RTPAmout_COUNT, 
			
			COALESCE(SUM(CASE WHEN lsh.statusId = 11 THEN LD.principal_amt END), 0) as CBAmout, 
			
			COUNT(CASE WHEN lsh.statusId = 11 THEN lsh.statusId ELSE NULL END) as CBAmout_COUNT, 
			
			COUNT(CASE WHEN lsh.statusId = 12 THEN lsh.statusId ELSE NULL END) as P3M_COUNT, 
			
			COUNT(CASE WHEN lsh.statusId = 13 THEN lsh.statusId ELSE NULL END) as ALD_COUNT, 
			
			COUNT(CASE WHEN lsh.statusId = 14 THEN lsh.statusId ELSE NULL END) as ATDC_COUNT, 
			
			COUNT(CASE WHEN lsh.statusId = 15 THEN lsh.statusId ELSE NULL END) as EMI_COUNT, 
			
			COUNT(CASE WHEN lsh.statusId = 16 THEN lsh.statusId ELSE NULL END) as PTPWIW_COUNT, 
			
			COUNT(CASE WHEN lsh.statusId = 17 THEN lsh.statusId ELSE NULL END) as PP_COUNT, 
			
			COUNT(CASE WHEN lsh.statusId = 18 THEN lsh.statusId ELSE NULL END) as RBI_COUNT, 
			
			COUNT(CASE WHEN lsh.statusId = 20 THEN lsh.statusId ELSE NULL END) as CASH_COUNT, 
			
			(COUNT(CASE WHEN lsh.statusId = 1 THEN lsh.statusId ELSE NULL END) + COUNT(CASE WHEN lsh.statusId = 2 THEN lsh.statusId ELSE NULL END) + COUNT(CASE WHEN lsh.statusId = 3 THEN lsh.statusId ELSE NULL END) + COUNT(CASE WHEN lsh.statusId = 4 THEN lsh.statusId ELSE NULL END) + COUNT(CASE WHEN lsh.statusId = 7 THEN lsh.statusId ELSE NULL END) + COUNT(CASE WHEN lsh.statusId = 8 THEN lsh.statusId ELSE NULL END) + COUNT(CASE WHEN lsh.statusId = 9 THEN lsh.statusId ELSE NULL END) + COUNT(CASE WHEN lsh.statusId = 10 THEN lsh.statusId ELSE NULL END) + COUNT(CASE WHEN lsh.statusId = 11 THEN lsh.statusId ELSE NULL END) + COUNT(CASE WHEN lsh.statusId = 12 THEN lsh.statusId ELSE NULL END) + COUNT(CASE WHEN lsh.statusId = 13 THEN lsh.statusId ELSE NULL END) + COUNT(CASE WHEN lsh.statusId = 14 THEN lsh.statusId ELSE NULL END) + COUNT(CASE WHEN lsh.statusId = 15 THEN lsh.statusId ELSE NULL END) + COUNT(CASE WHEN lsh.statusId = 16 THEN lsh.statusId ELSE NULL END) + COUNT(CASE WHEN lsh.statusId = 17 THEN lsh.statusId ELSE NULL END) + COUNT(CASE WHEN lsh.statusId = 18 THEN lsh.statusId ELSE NULL END) + COUNT(CASE WHEN lsh.statusId = 20 THEN lsh.statusId ELSE NULL END)) as callCount, 
			
			COALESCE(COALESCE(SUM(CASE WHEN LD.is_assigned = 1 THEN LD.principal_amt ELSE 0 END), 0) - COALESCE(SUM(CASE WHEN lsh.statusId = 5 THEN LD.principal_amt END), 0)) as remainingAmount, 
			
			(COUNT(CASE WHEN LD.is_assigned = 1 THEN LD.principal_amt ELSE NULL END) - COUNT(CASE WHEN lsh.statusId = 6 THEN lsh.statusId ELSE NULL END)) as remainingAmount_COUNT, 
			
			COALESCE(COALESCE(COUNT(CASE WHEN LD.is_assigned = 1 THEN LD.principal_amt ELSE NULL END)) - COALESCE((COUNT(CASE WHEN lsh.statusId = 1 THEN lsh.statusId ELSE NULL END) + COUNT(CASE WHEN lsh.statusId = 2 THEN lsh.statusId ELSE NULL END) + COUNT(CASE WHEN lsh.statusId = 3 THEN lsh.statusId ELSE NULL END) + COUNT(CASE WHEN lsh.statusId = 4 THEN lsh.statusId ELSE NULL END) + COUNT(CASE WHEN lsh.statusId = 7 THEN lsh.statusId ELSE NULL END) + COUNT(CASE WHEN lsh.statusId = 8 THEN lsh.statusId ELSE NULL END) + COUNT(CASE WHEN lsh.statusId = 9 THEN lsh.statusId ELSE NULL END) + COUNT(CASE WHEN lsh.statusId = 10 THEN lsh.statusId ELSE NULL END) + COUNT(CASE WHEN lsh.statusId = 11 THEN lsh.statusId ELSE NULL END) + COUNT(CASE WHEN lsh.statusId = 12 THEN lsh.statusId ELSE NULL END) + COUNT(CASE WHEN lsh.statusId = 13 THEN lsh.statusId ELSE NULL END) + COUNT(CASE WHEN lsh.statusId = 14 THEN lsh.statusId ELSE NULL END) + COUNT(CASE WHEN lsh.statusId = 15 THEN lsh.statusId ELSE NULL END) + COUNT(CASE WHEN lsh.statusId = 16 THEN lsh.statusId ELSE NULL END) + COUNT(CASE WHEN lsh.statusId = 17 THEN lsh.statusId ELSE NULL END) + COUNT(CASE WHEN lsh.statusId = 18 THEN lsh.statusId ELSE NULL END) + COUNT(CASE WHEN lsh.statusId = 20 THEN lsh.statusId ELSE NULL END)))) as remainingCalls, 
			
			COALESCE(((COALESCE(SUM(CASE WHEN lsh.statusId = 6 THEN LD.principal_amt END), 0) / COALESCE(SUM(CASE WHEN LD.is_assigned = 1 THEN LD.principal_amt ELSE 0 END), 0)) * 100), 0) as inPercentage 
